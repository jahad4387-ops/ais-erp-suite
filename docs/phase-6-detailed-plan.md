...
> 当前落地边界：Agent Action 在创建时保存 `evidenceSnapshots`；每次 Approve 前和 Execute 前都会复验附件状态与 checksum。校验失败时流程停留在原状态，不写入半截审批或执行结果，并同时生成 `evidence_verification_failed` Replay 事件和高危 `security_events`。
...
当前落地边界：草稿生成已在 LLM 输出后执行系统侧主数据匹配，覆盖采购/销售伙伴、存货物料、仓库、会计科目、工资批次和固定资产卡片。匹配结果写入 `matchedMasterData`，未匹配或歧义字段写入 `unmatchedItems`，转换为正式系统草稿前必须由人工确认或补齐，避免直接信任模型生成的业务 ID。
...
- **当前落地边界**：安全事件已从普通操作日志中独立出来，提供 `security_events` 持久化表和 `/api/security/events` 查询接口。当前会结构化记录权限拒绝、幂等键跨账套复用、Agent 执行前证据 Hash 校验失败等异常；证据被篡改时执行会被阻断，同时写入 `evidence_verification_failed` Replay 事件和 `severity=high` 的安全事件。安全事件字段包括事件类型、严重级别、操作者、账套、对象、消息和 JSON payload；查询接口支持按账套、事件类型、严重级别和操作者筛选，并继续遵守账套访问隔离。
...
| 页面 | 设计重点 |
| --- | --- |
| Agent 动作列表 | 展示任务类型、风险等级、状态、发起人、目标账套、期间、更新时间；列表 API 必须支持 `page` / `pageSize` 分页，避免生产队列全量拉取 |
...
| 安全审计 | 越权拦截、敏感操作、环境模式、幂等键异常、证据 Hash 校验失败和性能压测结果；已落地 `security_events` 与 `/api/security/events`，支持权限拒绝、幂等范围异常和 Agent 证据篡改高危事件的持久化筛选 |
...
- 运维安全审计页必须展示 `evidence_verification_failed`、严重级别、操作者、对象 ID 和 JSON payload，方便审计人员追查证据篡改来源。
