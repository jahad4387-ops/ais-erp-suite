...
test("Phase 1 write endpoints require idempotency and risk metadata", () => {
  const writePaths = [
    ...
    "/production-plans:",
    "/production-plans/{productionPlanId}/generate-work-orders:",
    "/rework-orders:",
    "/rework-orders/{reworkOrderId}/approve:",
    "/outsourcing-orders:",
    "/trace-rules:",
    "/traceability/query:",
...
test("Phase 1 contract exposes schemas needed by backend, frontend, and Agent tools", () => {
  const requiredSchemas = [
    ...
    "LineSideWarehouseConfigList:",
    "LineSideMaterialMovement:",
    "LineSideMaterialMovementList:",
    "WorkOrder:",
    "MaterialRequisition:",
...
test("Agent-facing endpoints expose dry-run and evidence fields", () => {
  ...
  assert.match(contract, /AGENT_DRAFT_PERIOD_CLOSED/, "Agent draft conversion must document closed-period protection.");
  assert.match(contract, /production_plan_draft/, "Agent draft conversion must document production plan previews.");
  assert.match(contract, /line_side_replenishment_draft/, "Agent draft conversion must document line-side replenishment previews.");
